...
std::vector<std::vector<RelativeIndex>> SearchServer::Search(const std::vector<std::string>& queries_input, int responses_limit)
{
    ...
    for (auto query : queries_input) {
        ...
        // additional loading map, value - docs_info [doc_id, word's ABS_rank]
        for (auto& [word, docs_info] : map_sort_by_word)
            docs_info = _index.GetWordCount(word);

        // load new map, key - doc_id, value - doc's ABS_rank (sum of word's ABS_rank for all words)
        ...
        if (!map_resort_by_doc_id.empty()) {
            ...
            // load nem multimap, key - doc's ABS_rank, value - doc_id
            ...
            // relevance calculation for each doc_id according responses_limit
